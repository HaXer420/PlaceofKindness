...
exports.signupneedy = catchAsync(async (req, res, next) => {
  if (!req.body.cnic) {
    return next(
      new AppError(
        'Please enter cnic or else you cannot create needy Account',
        400
      )
    );
  }
  ...
  await sendEmail({
    email: newUser.email,
    subject: 'Welcome to the POK Family',
    message,
  });
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  const resetURL = `http://localhost:3000/resetforgetpassword?token=${resetToken}`;
...
exports.sendEmailConfirm = catchAsync(async (req, res, next) => {
  ...
  const resetURL = `${req.protocol}://${req.get(
    'host'
  )}/api/v1/users/emailConfirm/${EmailToken}`;

  const message = `Welcome to the Place of Kindness! Click on the given link to verify your POK account:  ${resetURL} \n If you dont do this please ignore this email`;
  ...
  try {
    ...
    res.status(200).json({
      ...
      token: req.token,
      message:
        'Welcome to POK family! Account Verificiation sent to your email',
    });
  } catch (err) {
    user.passwordResetToken = undefined;
    user.passTokenExpire = undefined;
